...
const ANALYSIS_STORAGE_KEY = "DeepShieldLatestAnalysis";
const SETTINGS_STORAGE_KEY = "DeepShieldSettings";
const MODE_CONTROL_KEY = "DeepShieldControl"; // Added for Play/Pause
...
console.log("DeepShield: Content script initialized.");
console.log(
  "DeepShield: Frontend development setup active. Expecting temporary backend responses from http://127.0.0.1:5000."
);
...
function applyNewSettings(newSettings) {
  ...
  console.log("DeepShield: Settings updated to", activeSettings);
...
function applyControlState(controlState) {
  ...
  if (controlState.isRunning !== isRunning) {
    ...
    console.log(`DeepShield: Detection state changed. isRunning = ${isRunning}`);
...
async function checkBackendConnection() {
  try {
    ...
    if (lastConnectionState !== "connected") {
      console.log(
        `DeepShield: Backend connected. Mode: ${health.mode || "unknown"}. Temporary result: ${health.temporaryResult || "no"}.`
      );
    ...
    if (lastConnectionState !== "disconnected") {
      console.warn(
        `DeepShield: Backend disconnected. Start the server at http://127.0.0.1:5000 before testing the popup. ${error.message}`
      );
...
function captureFrameFromVideo(videoElement) {
  ...
  try {
    ...
    console.error("DeepShield: Error capturing frame:", error);
    return null;
  }
}
...
async function startApp() {
  console.log("DeepShield Content Script Loading Settings...");
